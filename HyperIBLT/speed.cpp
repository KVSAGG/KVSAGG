...
int main(){
    ...
    for(int j=0;j<1;j++){
        ...
        for(int k=0;k<4;k++){
            int numHyperIBLT=(int)((double)aaa[k]*1.25/3.0);
            int numCS=(int)((double)numHyperIBLT*12.5/8.0);
            int numCSK=(int)((numHyperIBLT*12.5*3-(double)aaa[k]*4)/8.0/3.0);
                
            double ans_insert=0,ans_decode=0;
            int repeat=10000000/aaa[k];
            if(j==0){
                for(int t=0;t<repeat;t++){
                    Fermat ferm(3,numHyperIBLT,0,t);
